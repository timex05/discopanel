...
class Completions {
    ...
    private findNextSubcommand(commandObject: CommandObject, token: string): CommandObject | undefined {
        ...
        const argumentMatch = commandObject.subcommands.find((subcommand) => subcommand.type === 'argument');
        if (argumentMatch) {
    ...
    private shouldIgnoreRepeatedOptionalTail(commandObject: CommandObject, tailTokens: string[], rootCommand?: string): boolean {
    ...
    private async collectChildCompletions(commandObject: CommandObject, currentToken: string): Promise<string[]> {
        const output: string[] = [];
        ...
        if (hasTargetPlaceholder) {
            ...
            const players = this.extractOnlinePlayers(listOutput).filter((name) =>
                currentToken === '' || name.startsWith(currentToken)
            );
            output.push(...players);
        }
